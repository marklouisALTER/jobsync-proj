...
export default function SocmedSettings() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (selectedSocialLinks.length === 0) {
      Swal.fire({
        icon: 'error',
        title: 'Oops...',
        text: 'No valid social media links to save!',
      });
      return;
    }
    try {
      const response = await postToEndpoint('/socialmedia.php', {
        employer_id: user?.id,
      ...
      if (response.data.status === 'success') {
        Swal.fire({
          icon: 'success',
          title: 'Success',
          text: 'Social media links have been saved successfully!',
      ...
      Swal.fire({
        icon: 'error',
        title: 'Error',
        text: `Error saving social links: ${error.message}`,
        showConfirmButton: false,
        timer: 1000,
        timerProgressBar: true,
        allowOutsideClick: false,
  ...
  return (
    <Container fluid className="text-start" style={{ margin: 0, width: '100%', padding: '15px 8px' }}>
