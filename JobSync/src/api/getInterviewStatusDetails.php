...
try {
    ...
    $query = "SELECT a.*, s.schedule, s.time, s.channel_name, s.status, ap.profile_picture, s.created_at FROM view_applications a
    JOIN js_interview_schedule s ON a.application_id = s.application_id
    JOIN js_applicants ap ON a.applicant_id = ap.applicant_id
    WHERE a.employer_id = :employer_id AND a.application_id = :application_id ORDER BY created_at DESC";
