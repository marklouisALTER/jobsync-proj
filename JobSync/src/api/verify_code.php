...
if ($method == 'POST') {
    if (isset($_POST["email"]) && isset($_POST["verification_code"]) && isset($_POST["formType"])) {
        ...
        switch ($formType) {
            case 'applicant':
                $checkSql = "SELECT * FROM js_applicants WHERE email = :email AND verification_code = :verification_code";
                break;
            case 'employer':
                $checkSql = "SELECT * FROM js_employer_info WHERE email = :email AND verification_code = :verification_code";
                break;
            default:
                $response = [
                    "status" => 0,
        ...
        $checkStmt->bindParam(':verification_code', $verification_code);
        $checkStmt->execute();
        $user = $checkStmt->fetch(PDO::FETCH_ASSOC);
    
        if ($user && isset($user['email_verified_at']) && $user['email_verified_at'] === 'verified') {
            $response = [
                "status" => 0,
            ...
            switch ($formType) {
                case 'applicant':
                    if ($stmt->rowCount() > 0) {
                        $updateSql = "UPDATE js_applicants SET email_verified_at = 'verified' WHERE email = :email";
